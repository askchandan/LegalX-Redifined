...
export default function Home() {
  ...
  const sendMessage = async () => {
    ...
    setInput("");
    try {
      // Build API endpoint robustly — NEXT_PUBLIC_API_URL may be undefined in dev/build
      const apiBase = process.env.NEXT_PUBLIC_API_URL || "";
      if (!process.env.NEXT_PUBLIC_API_URL) {
        // fall back to internal API route when no external URL is provided
        console.warn("NEXT_PUBLIC_API_URL is not set — falling back to /api/generate");
      }
      const endpoint = apiBase ? `${apiBase}/generate` : "/api/generate";
